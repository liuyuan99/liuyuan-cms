			...
			<tr>
				...
				<td>
					...
					<c:if test="${user.locked==0}">
						<button type="button" class="btn btn-info" onclick="update(${user.id},this)">正常</button>
					</c:if>
					<!-- 用户禁用状态 -->
					<c:if test="${user.locked==1}">
						<button type="button" class="btn btn-danger" onclick="update(${user.id},this)">禁用</button>
					</c:if>
				</td>
			</tr>
		</c:forEach>
		...
		function update(id,obj){
			...
			$.post("/admin/updateUser",{id:id,locked:locked},function(flag){
				if(flag){
					...
					$(obj).attr("class",locked==0?"btn btn-info":"btn btn-danger")
				}
			})
		}
